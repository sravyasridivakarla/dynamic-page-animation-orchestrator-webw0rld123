"""SQLAlchemy models for Page Config Drafter."""
...
class PageConfig(Base):
    ...
    scroll_sections = relationship(
        "ScrollSection", back_populates="page_config", cascade="all, delete-orphan"
    )
    visual_representations = relationship(
        "VisualRepresentation", back_populates="page_config", cascade="all, delete-orphan"
    )
    sourcing_timeline_events = relationship(
        "SourcingTimelineEvent", back_populates="page_config", cascade="all, delete-orphan"
    )
    provenance_records = relationship(
        "ProvenanceRecord", back_populates="page_config", cascade="all, delete-orphan"
    )
...
class ScrollSection(Base):
    ...
    page_config_id = Column(
        UUID(as_uuid=True), ForeignKey("page_configs.id", ondelete="CASCADE"), nullable=False
    )
...
class ProvenanceRecord(Base):
    ...
    diff_presented = Column(JSON, nullable=False)
